...
class RAFTOpticalFlowProcess(dataprocess.CVideoTask):
    ...
    def trained_model(small, device, dropout=0, mixed_precision=True, alternate_corr=False):
        ...
        if device == "cuda":
            ...
            if device == "cpu":
                ...
                pretrained_weights = RAFTOpticalFlowProcess.get_cpu_model(pretrained_weights)
                # load the pretrained weights into model
                model.load_state_dict(pretrained_weights)
    ...
    def flow_from_images(model, device, frame_1, frame_2):
        # frame preprocessing
        frame_1 = RAFTOpticalFlowProcess.frame_preprocess(frame_1, device)
        frame_2 = RAFTOpticalFlowProcess.frame_preprocess(frame_2, device)
        with torch.no_grad():
            # predict the flow
    ...
    def run(self):
        ...
        if self.frame_1 is not None:
            ...
            frame_1 = srcImage
            flow = RAFTOpticalFlowProcess.flow_from_images(param.model, param.device, frame_1, frame_2)
            img_flo = RAFTOpticalFlowProcess.vizualize_flow(flow)
...
class RAFTOpticalFlowProcessFactory(dataprocess.CTaskFactory):
    ...
    def create(self, param=None):
        ...
        return RAFTOpticalFlowProcess(self.info.name, param)
